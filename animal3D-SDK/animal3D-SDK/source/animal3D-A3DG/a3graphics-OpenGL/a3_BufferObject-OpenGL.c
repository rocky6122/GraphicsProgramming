...
a3ret a3bufferFill(a3_BufferObject *buffer, a3i32 section, const a3ui32 size, const void *data, a3ui32 *start_out_opt)
{
	a3ui32 bHandle, start, end;

	if (buffer && size)
	{
		bHandle = buffer->handle->handle;
		if (bHandle)
		{
			// check if data will fit
			section = section ? 1 : 0;
			start = buffer->used[section];
			end = start + size;
			if (end <= buffer->split[section])
			{
				// bind and fill
...
a3ret a3bufferFillOffset(a3_BufferObject *buffer, a3i32 section, const a3ui32 offset, const a3ui32 size, const void *data, a3ui32 *start_out_opt)
{
	a3ui32 bHandle, start, end;
	...
	if (buffer && size)
	{
		...
		if (bHandle)
		{
			// check if data will fit
			section = section ? 1 : 0;
			start = buffer->used[section] + offset;
			end = start + size;
			if (end <= buffer->split[section])
			{
				// bind and fill
