...
//		break;
	...
	case 'y':
		if (demoState->demoMode != 1) //If not in DrawCurve demo mode
		{
		}
		else if (demoState->animSpeed > 1)
		{
			demoState->animSpeed -= 1;
		}
		break;
	...
	case 'Y':
		...
		else if (demoState->animSpeed < demoStateMaxAnimation_Speed)
		{
			demoState->animSpeed += 1;
		}
		break;
	...
	case 'u':
		...
		{
			if (demoState->splineType)
			{
				demoState->splineType = 0;
			}
			else
			{
				demoState->splineType = 1;
			}
		}
		break;
	}
	...
	if (demoState->demoMode == 1)
	{
		if (button == a3mouse_left)
		{
			if (demoState->curveWaypointCount < demoStateMaxCount_curveWaypoint)
			{
				...
				//Midterm add speedIndex for this curve
				if (demoState->curveWaypointCount > 0)
				{
					//Add speed for curve
					demoState->segmentSpeed[demoState->curveWaypointCount - 1] = demoState->animSpeed;

					demoState->lineTotalLength = 0;

					//Calculate percentages for all curve segments
					for (int i = 0; i < (int)demoState->curveWaypointCount; ++i)
					{

						if (demoState->splineSegment[i] == 0)
						{
							//Catmull-Rom ArcLength Calculations
							a3real3CalculateArcLengthCatmullRom(&demoState->curveArcLengthSamples->v, //array of samples captured while calculating length
								demoState->curveArcLengthInterpolationParams, //array of interpolation params matched to samples
								demoState->curveArcLengthOutputs, //array of accumulated arc lengths at each sample
								0, //options to autonormalize
								31, //Number of intermediate samples on curve segment (number of divisions + 1 is allocated)
								demoState->curveWaypoint[a3maximum(0, i - 1)].v, //Previous Control Vector
								demoState->curveWaypoint[i + 0].v, //Initial Control vector
								demoState->curveWaypoint[i + 1].v, //Goal/End Control vector
								demoState->curveWaypoint[a3minimum((a3ui32)i + 2, (a3ui32)demoState->curveWaypointCount - 1)].v //Next Control vector
							);
							...
							for (int j = 0; j < 32; ++j)
							{
								temp += demoState->curveArcLengthOutputs[j];
							}

							demoState->segmentLength[i] = temp;

							demoState->lineTotalLength += demoState->segmentLength[i];
						}
						else
						{
							//Cubic Hermite ArcLength Calculations
							a3real3CalculateArcLengthHermiteControl(&demoState->curveArcLengthSamples->v, //array of samples captured while calculating length
								demoState->curveArcLengthInterpolationParams, //array of interpolation params matched to samples
								demoState->curveArcLengthOutputs, //array of accumulated arc lengths at each sample
								0, //options to autonormalize
								31, //Number of intermediate samples on curve segment (number of divisions + 1 is allocated)
								demoState->curveWaypoint[i + 0].v, //Initial control vector
								demoState->curveWaypoint[i + 1].v, //Goal/End Control vector
								demoState->curveHandle[i + 0].v, //Control Handle corresponding to v0
								demoState->curveHandle[i + 1].v //Control handle correspongering to v1
							);
				...
				//Final add splineType for this curve
				if(demoState->curveWaypointCount > 0)
				demoState->splineSegment[demoState->curveWaypointCount - 1] = demoState->splineType;
		...
		else if (button == a3mouse_right)
		{
			...
			{
				//Midterm add speedIndex for this curve
				demoState->segmentSpeed[demoState->curveWaypointCount - 1] = 0;

				demoState->lineTotalLength -= demoState->segmentLength[demoState->curveWaypointCount - 1];

				demoState->segmentLength[demoState->curveWaypointCount - 1] = 0;
